...
{
  case 'testnet':
    network = berycoin.networks.berycoin_testnet
    break
  case 'mainnet':
    network = berycoin.networks.berycoin
    break
}
...
export default class Wallet {
  ...
  static generateCreateContractTx(wallet, code, gasLimit, gasPrice, fee, utxoList) {
    return berycoin.utils.buildCreateContractTransaction(wallet.keyPair, code, gasLimit, gasPrice, fee, utxoList)
  }
  ...
  static async generateSendToContractTx(wallet, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList) {
    ...
    return berycoin.utils.buildSendToContractTransaction(wallet.keyPair, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList)
  }
  ...
  static async generateTx(wallet, to, amount, fee, utxoList) {
    ...
    return berycoin.utils.buildPubKeyHashTransaction(wallet.keyPair, to, amount, fee, utxoList)
  }
  ...
  static restoreFromMnemonic(mnemonic, password) {
    ...
    const hdNode = berycoin.HDNode.fromSeedHex(seedHex, network)
  ...
  static restoreFromWif(wif) {
    return new Wallet(berycoin.ECPair.fromWIF(wif, network))
  }

  static async restoreHdNodeFromLedgerPath(ledger, path) {
    const res = await ledger.berycoin.getWalletPublicKey(path)
    const compressed = ledger.berycoin.compressPublicKey(buffer.Buffer.from(res['publicKey'], 'hex'))
    const keyPair = new berycoin.ECPair.fromPublicKeyBuffer(compressed, network)
    const hdNode = new berycoin.HDNode(keyPair, buffer.Buffer.from(res['chainCode'], 'hex'))
    hdNode.extend = {
      ledger: {
        ledger,
        path
