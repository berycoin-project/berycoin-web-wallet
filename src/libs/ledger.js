import berycoinJsLib from 'berycoinjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'berycoin-opcodes'
...
export default class Ledger {
  ...
  static async connect() {
    const berycoin = new btcApp(await transportU2f.create())
    // ensure in Berycoin App
    const pubkeyRes = await berycoin.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
      throw 'Not Berycoin App'
    }
    return new Ledger(berycoin)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.berycoin.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = berycoinJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.berycoin.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new berycoinJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.berycoin.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new berycoinJsLib.TransactionBuilder(keyPair.network)
    const contract =  berycoinJsLib.script.compile([
      OPS.OP_4,
